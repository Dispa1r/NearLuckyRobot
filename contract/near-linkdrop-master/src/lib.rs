...
const ACCESS_MIN_MONEY: u128 = 100_000_000_000_000_000_000_000;
...
mod tests {
    ...
    fn test_send_luck() {
        let mut contract = LinkDrop::default();
        let mut v:Vec<Base58PublicKey> = Vec::new();
        let pk: Base58PublicKey = "qSq3LoufLvTCTNGC3LJePMDGrok8dHMQ5A1YD9psbiz"
            .try_into()
            .unwrap();
        v.push(pk.clone());
        ...
        v.push(pk.clone());
        v.push(pk.clone());
        let deposit = ONE_NEAR * 10;
        testing_env!(VMContextBuilder::new()
            .current_account_id(linkdrop())
            .attached_deposit(deposit)
            .finish());
        contract.send_luck(v,5);
    }
    ...
    fn test_claim_invalid_account() {
        ...
        let pk2 = "2S87aQ1PM9o6eBcEXnTR5yBAVRTiNmvj8J8ngZ6FzSca"
            .try_into()
            .unwrap();
        contract.create_account_and_claim("bob".to_string(), pk2);
    }
